...
This example checks the correct application of a load
on the deformed configuration of a surface of the domain.
...
public:
    ...
    void ComputeBC( const boost::shared_ptr<RegionMesh<LinearTetra> > localMesh,
                    const VectorEpetra& disp,
                    boost::shared_ptr<VectorEpetra> bcVectorPtr,
                    const solidETFESpacePtr_Type dETFESpace,
                    int bdFlag);
...
void Structure::ComputeBC( const boost::shared_ptr<RegionMesh<LinearTetra> > localMesh,
                const VectorEpetra& disp,
                boost::shared_ptr<VectorEpetra> bcVectorPtr,
                const Structure::solidETFESpacePtr_Type dETFESpace,
                int bdFlag)
{
	...
	*bcVectorPtr *= 0.0;
...

	{
        ...
        integrate ( boundary ( localMesh, bdFlag),
        		    myBDQR,
        		    dETFESpace,
        		    det( grad( dETFESpace, disp, 0) + value(Id) )  * dot( minusT(grad( dETFESpace, disp, 0) + value(Id))  * Nface,  phi_i)
                  ) >> bcVectorPtr;
...
Structure::run3d()
{
    ...
    vectorPtr_Type boundaryVectorPtr(new vector_Type ( dFESpace->map(), Repeated ) );
    vector_Type solidDispInit (dFESpace->map(),  Unique );
    solidDispInit *= 0.0;
    Structure::ComputeBC(dFESpace->mesh(), solidDispInit, boundaryVectorPtr, dETFESpace, 20);
    boost::shared_ptr<BCVector> bcVectorPtr( new BCVector (*boundaryVectorPtr, dFESpace -> dof().numTotalDof(), 0 ) );
    BCh->addBC("DeformedSide", 20, Natural, Full, *bcVectorPtr, 3);
    //BCh->addBC ("EdgesIn",      20,  Natural, Full, pressure, 3);
    ...
    *solidAcc = timeAdvance->secondDerivative();
    ...
    for (Real time = initialTime + dt; time <= T; time += dt)
    {
        ...
        *rhs += *solid.massMatrix() * timeAdvance->rhsContributionSecondDerivative() / timeAdvanceCoefficient;
        ...
        // Updating BC
        Structure::ComputeBC(dFESpace->mesh(), solid.displacement(), boundaryVectorPtr, dETFESpace, 20);
        *boundaryVectorPtr *= 100000;
        bcVectorPtr.reset( new BCVector (*boundaryVectorPtr, dFESpace->dof().numTotalDof(), 0 ) );
	    solid.bcH()->modifyBC(20, *bcVectorPtr);
