...
int main(int argc, const char * argv[]) {
    ...
    for ( unsigned int i (0) ; i < tEnd / dt ; ++i )
    {
        ...
        while ( !bc.coupling().converged(VFeNew, VCircNew, 1e-5) )
        {
            ++iter;
            ...
            // update pressure
            Vec<double> dpPert (2, dp);
            bc.coupling().updatePressure(bcValues, VFeNew, VFEPert, dpPert, VCircNew, VCircPert, dpPert);
