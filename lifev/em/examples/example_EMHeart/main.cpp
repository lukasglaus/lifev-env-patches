...
//        patchVecPtr.push_back(normalVectorPtr);
    ...
    auto modifyPatchBC = [&] (const Real& patchNormalDisp, const int& patchNr, const int& flag)
    {
        ...
        patchVecPtr[patchNr].reset( *normalEssentialBCVector(solver.fullMeshPtr(), solver.structuralOperatorPtr()->dispFESpacePtr(), solver.bcInterfacePtr()->handler()->findBCWithName(patchNames[patchNr])) );
